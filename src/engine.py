...
def run_quantization_study():
    ...
    # --- Step 1: まず理想状態(無限精度)で学習して「正解」を得る ---
    ...
    # --- Step 2: 量子化実験 (ビット数を変えてテスト) ---
    # DACのフルスケール範囲を仮定 (例: -1.0V ~ +1.0V の範囲を刻む = 幅2.0V)
    v_range = 2.0 
    
    # テストするビット数 (2bitから12bitまで)
    ...
    plt.yscale('log') # ログスケール (重要: エラーの桁を見るため)
    plt.grid(True, which="both", ls="-", alpha=0.5)
    
    # 許容ライン (Loss < 0.01 ならOKとする目安)
